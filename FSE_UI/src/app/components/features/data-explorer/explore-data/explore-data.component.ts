...
import exploreDataColumns from '../../../../../assets/columnDefinition/explore-data-columns.json';
...
@Component({
  ...
  styleUrl: './explore-data.component.scss'
})
export class ExploreDataComponent {
  ...
  constructor(private exploreDataService: ExploreDataService, private sharedService: SharedService,private router: Router,private nlqRestService: NlqRestService, private loaderService: LoaderService,
  ...
  //   this.exploreDataService.getExploreData(nlqRequest).subscribe((result) => {
  //     this.showGrid = true;
  ...
  async callNlqSearch(nlqRequest: NLQRequestEto) {
    ...
    const nlqResponse = await this.exploreDataService.getExploreData(nlqRequest).toPromise();
    if (
      nlqResponse &&
      nlqResponse?.data &&
  ...
  openExploreDocumentationDetails(event: any) {
    ...
    const dialogRef: DialogRef = this.dialogService.open({
      content: ExploreDocDialogComponent,
    });
